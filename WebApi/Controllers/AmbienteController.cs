...
using Application.Ambiente.Commands.Update;
using Application.Ambiente.Queries;
using Application.Ambiente.Queries.Obtener;
...
namespace SistemaInformatico.API.Controllers
{
    ...
    public class AmbienteController : AbstractController
    {
        ...
        public async Task<IActionResult> ObtenerSegmentacion([FromRoute] int Id)
        {
            ...
            try
            {
                var response = await Mediator.Send(new ObtenerAmbienteQuery { Id = Id });
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> UpdateVariablePaft([FromRoute] int datoID, [FromBody] UpdateAmbienteCommand command)
        {
