using Application.Documento.Commands.Crear;
using Application.Documento.Commands.Update;
using Application.Documento.Queries.Listar;
using Application.Documento.Queries.Obtener;
...
namespace SistemaInformatico.API.Controllers
{
    ...
    public class DocumentoController : AbstractController
    {
        ...
        public async Task<IActionResult> ObtenerSegmentacion([FromRoute] int Id)
        {
            ...
            try
            {
                var response = await Mediator.Send(new ObtenerDocumentoQuery { Id = Id });
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        [Route("agregar")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> CrearDocumento([FromBody] CreateDocumentoCommand command)
        {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> UpdateVariablePaft([FromRoute] int datoID, [FromBody] UpdateDocumentoCommand command)
        {
